...
public class WallRepulsion implements Force {
    ...
    public WallRepulsion (Wall wall, double magnitude, double exponent) {
        ...
        System.out.printf("Wall Repulsion created for wall #%d w/ mag = %.2f, exp = %.2f ", mWallId, magnitude, exponent);
        System.out.printf("at JGAME (%.2f, %.2f) \t JBOX (%.2f, %.2f) \n", wall.x, wall.y, wall.getBody().getPosition().x, wall.getBody().getPosition().y);
    }
...
//        
    ...
    private void testWallRepulsion(Mass mass, Vec2 force, float scalar) {
        System.out.printf("\tCalculating wall force: Position=(%.2f, %.2f), Wall direction=%d, angle=%.2f\n", 
                          mass.getBody().getPosition().x, mass.getBody().getPosition().y, this.mWallId, this.mDirection);
